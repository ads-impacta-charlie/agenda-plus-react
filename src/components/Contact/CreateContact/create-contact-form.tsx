...
import CreateContactData from "@/components/Contact/CreateContact/create-contact-data";
import { Button, Grid } from "@mui/material";
import { Contact } from "@/entity/contact";
import CreateContactDataMenu from "@/components/Contact/CreateContact/create-contact-data-menu";
import { v4 } from "uuid";
...
export default function CreateContactForm({
  ...
  const handleSaveClick = () => {
    ...
    }).then((jsonResponse: any) => onSave(jsonResponse as Contact));
  };
      ...
      {data.map((d, i) => (
        <Grid key={d.uuid} container spacing={2}>
          ...
          <Grid item xs={12} md={2}>
            <CreateContactDataMenu
              data={d}
              index={i}
              onDelete={handleOnDeleteContactData}
            />
          </Grid>
        </Grid>
      ))}
      <Button onClick={handleAddDataClick} variant="outlined">
        Adicionar Dados
      </Button>
